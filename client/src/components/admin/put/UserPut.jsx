...
import { Button, Input } from "../post/index";
...
function UserPut() {
  ...
  // Function bach njibo token mn localStorage
  const getAuthHeader = () => {
  ...
  useEffect(() => {
    axios.get(LINK + "/" + id, { headers: getAuthHeader() })
      .then((res) => {
        setData({
          nom: res.data.nom || "",
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put(LINK + "/" + id, data, { headers: getAuthHeader() });
